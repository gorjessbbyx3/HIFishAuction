...
class OceanService:
    ...
    def get_current_ocean_conditions(self) -> Dict:
        """Get current ocean conditions around Hawaii"""
        try:
            sst_data = self._get_sea_surface_temperature()
            chlorophyll_data = self._get_chlorophyll_levels()
            current_data = self._estimate_current_conditions()
            
            ocean_conditions = {
                'sea_surface_temp': sst_data.get('temperature', 26.5),
                'chlorophyll': chlorophyll_data.get('chlorophyll', 0.15),
                'current_strength': current_data.get('strength', 'Moderate'),
                'current_direction': current_data.get('direction', 'Southwest'),
            ...
            print(f"Error fetching ocean conditions: {str(e)}")
